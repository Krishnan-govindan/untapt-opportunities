          ...
          if (line.startsWith("event: ")) {
            ...
            try {
              ...
              if (currentEvent === "status") {
                setPhase((prev) =>
                  prev.kind === "running"
                    ? { kind: "running", messages: [...prev.messages, data.message as string] }
                    : prev
                );
    ...
    <main className="mx-auto max-w-4xl px-6 py-16">
      ...
      <div className="mb-12 text-center">
        ...
        <p className="mt-4 text-base text-muted-foreground max-w-2xl mx-auto">
          Enter a niche, industry, or product category — we'll search X, Quora, and
          LinkedIn for real complaints and surface the best unmonetized opportunities.
        </p>
      </div>
        ...
        <div>
          <div className="mb-6 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-foreground">
                {phase.opportunities.length} opportunit{phase.opportunities.length === 1 ? "y" : "ies"} found
              </h2>
          ...
          {phase.opportunities.length === 0 ? (
            <div className="rounded-xl border border-border bg-card p-10 text-center text-sm text-muted-foreground">
              No opportunities were found. Try a more specific niche or different keywords.
            </div>
          ) : (
      ...
      {phase.kind === "idle" && (
        <div className="rounded-xl border border-dashed border-border bg-card/50 p-12 text-center">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            {["dental billing", "HR onboarding", "freight logistics", "legal discovery", "school admin"].map(
              (ex) => (
                <button
                  key={ex}
                  onClick={() => setTopic(ex)}
