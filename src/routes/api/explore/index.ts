...
const RELEVANCE_KWS = [
  ...
  "there's no", 'please build', 'build this', 'problem', 'challenge',
  'pain point', 'struggle', 'difficult', 'impossible', 'workaround',
]
...
async function runExplorePipeline(
  ...
  try {
    ...
    if (filtered.length === 0) {
      send('error', { message: 'No relevant pain points found for this topic. Try a more specific industry or product category.' })
      return
    }
    ...
    if (allOpps.length === 0) {
      send('error', { message: 'AI found no structured opportunities in these results. Try a different topic.' })
      return
    }
...
export const APIRoute = createAPIFileRoute('/api/explore')({
