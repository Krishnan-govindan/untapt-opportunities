...
const POLITICAL_KWS = ['trump','biden','democrat','republican','election','vote','congress','maga'];
const RELEVANCE_KWS = ['workflow','tool','software','app','platform','automate','wish','hate','tired','frustrated','annoying','painful','manual','tedious','broken','integration','dashboard','report','track','manage','process','productivity','inefficient','should exist','missing feature','no solution',"can't find","why doesn't","someone should","nightmare","i wish","there's no","please build","pain point","struggle"];
const RESEARCH_DOMAINS = ['ncbi.nlm.nih.gov','researchgate.net','pubmed','arxiv.org','springer.com','ieee.org','sciencedirect.com'];
...
function fpHash(text) {
  return createHash('sha256').update(text.toLowerCase().replace(/\s+/g,' ').slice(0,200)).digest('hex').slice(0,16);
}
...
async function handleExplore(req, res) {
  ...
  try {
    ...
    if (!topic) { send('error', { message: 'topic is required' }); res.end(); return; }

    const apifyToken = process.env.APIFY_TOKEN;
    const anthropicKey = process.env.ANTHROPIC_API_KEY;
    if (!apifyToken || !anthropicKey) { send('error', { message: 'Missing API keys' }); res.end(); return; }
    ...
    const [twResult, gResult] = await Promise.allSettled([
      (async () => {
        ...
        return items.filter(i => i.text || i.full_text).map(i => ({
          platform: 'twitter', title: String(i.text ?? i.full_text ?? '').slice(0,120),
          content: String(i.text ?? i.full_text ?? ''), url: String(i.url ?? `https://x.com/i/web/status/${i.id ?? ''}`), score: Number(i.likeCount ?? 0),
        }));
      })(),
      (async () => {
        const run = await apify.actor('apify/google-search-scraper').call(
          { queries: googleQueries.join('\n'), maxPagesPerQuery: 1, resultsPerPage: 10, countryCode: 'us' },
          { waitSecs: 120 }
        );
        const { items } = await apify.dataset(run.defaultDatasetId).listItems();
        const results = [];
        for (const page of items) {
          for (const r of (page.organicResults ?? [])) {
            ...
            results.push({ platform: 'google', title: String(r.title ?? ''), content: String(r.description ?? r.snippet ?? ''), url: String(r.url ?? r.link ?? ''), score: 0 });
          }
        }
        return results;
      })(),
    ...
    if (!filtered.length) { send('error', { message: 'No relevant pain points found. Try a more specific niche.' }); res.end(); return; }
    ...
    for (let i = 0; i < filtered.length; i += BATCH) {
      ...
      const userContent = batch.map((item, idx) =>
        `[${idx+1}] Platform: ${item.platform}\nTitle: ${item.title}\nContent: ${item.content.slice(0,500)}\nURL: ${item.url}`
      ).join('\n\n---\n\n');
      try {
        const msg = await anthropic.messages.create({
          model: 'claude-opus-4-7', max_tokens: 4096, system: SYSTEM_PROMPT,
          messages: [{ role: 'user', content: `Extract startup opportunities from these ${batch.length} posts:\n\n${userContent}` }],
        });
        const raw = msg.content.filter(b => b.type === 'text').map(b => b.text).join('');
        const match = raw.match(/\[[\s\S]*\]/);
        if (match) {
          try { allOpps.push(...JSON.parse(match[0])); } catch {}
        }
      } catch {}
    }

    if (!allOpps.length) { send('error', { message: 'AI found no structured opportunities. Try a different topic.' }); res.end(); return; }

    send('status', { message: `Saving ${allOpps.length} opportunities to feed…` });

    const rows = allOpps.filter(o => o.title && o.pain && typeof o.urgency_score === 'number').map(o => ({
      title: o.title.slice(0, 200), pain_summary: o.pain, pain_description: o.pain,
      icp: o.icp ?? 'B2B professionals', tam_estimate: formatTAM(o.market_size_usd ?? 0),
      urgency_score: Math.min(10, Math.max(1, Math.round(o.urgency_score))),
      why_now: o.why_now ?? '', competitors: o.competitors ?? [], mvp_features: o.mvp_features ?? [],
      sources: [...new Set((o.sources ?? []).map(s => s.platform))].filter(Boolean),
      sources_detail: o.sources ?? [], is_hot: (o.urgency_score ?? 0) >= 8,
      dedup_hash: fpHash(o.title),
    }));

    await supabase.from('opportunities').upsert(rows, { onConflict: 'dedup_hash', ignoreDuplicates: true }).select('id');

    const hashes = rows.map(r => r.dedup_hash);
    const { data: saved } = await supabase.from('opportunities').select('*').in('dedup_hash', hashes);
